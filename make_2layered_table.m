function[]=make_2layered_table(theta,phi,MM1,MM2,td,nf)
  % td must be two dimensional [td1 td2] thicknesses
  % nf : figure number
  ...
  if pas~=1
    disp('Material 1 is not a passive medium.');
  ...
  pas=all(eig(Mc)>1e-8);  % check if all eigenvalues are positive
  if pas~=1
    disp('Material 2 is not a passive medium.');
    disp('Please provide appropriate material parameters');
    return;
  else
  end;
   
  [rss1, rps1, rsp1, rpp1, tss1, tps1, tsp1, tpp1]=fresnel_2layered_top(theta,phi,MM1,MM2,td);
  [rss2, rps2, rsp2, rpp2, tss2, tps2, tsp2, tpp2]=fresnel_2layered_top(theta,phi+pi,MM1,MM2,td);
  [rss3, rps3, rsp3, rpp3, tss3, tps3, tsp3, tpp3]=fresnel_2layered_bottom(theta,phi+pi,MM1,MM2,td);
  ...
  str3=strcat('(\theta,\phi)/\pi= (',num2str(theta/pi),' , ',num2str(phi/pi),')');
  title(str3);
