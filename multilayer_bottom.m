function[rss, rps, rsp, rpp, tss, tps, tsp, tpp]=multilayer_bottom(theta,phi)
...
% The unknowns are rss,rsp,rpp,rps, and 4 coefficients of up and down waves in each layer.
  ...
  % First 4 B.C.s 
  As(1,:)=[-esi(1) -epi(1) afd(1,1) bfd(1,1) afu(1,1) bfu(1,1) zeros(1,tN-6)];   % Ex continuity
  As(2,:)=[-esi(2) -epi(2) afd(2,1) bfd(2,1) afu(2,1) bfu(2,1) zeros(1,tN-6)];   % Ey continuity 
  As(3,:)=[-esi(4) -epi(4) afd(4,1) bfd(4,1) afu(4,1) bfu(4,1) zeros(1,tN-6)];   % Hx continuity 
  As(4,:)=[-esi(5) -epi(5) afd(5,1) bfd(5,1) afu(5,1) bfu(5,1) zeros(1,tN-6)];   % Hy
  ...
  As(tN-3,:)=[zeros(1,tN-6) pad*afd(1,N) pbd*bfd(1,N) pau*afu(1,N) pbu*bfu(1,N) -esr(1) -epr(1)];
  As(tN-2,:)=[zeros(1,tN-6) pad*afd(2,N) pbd*bfd(2,N) pau*afu(2,N) pbu*bfu(2,N) -esr(2) -epr(2)];
  As(tN-1,:)=[zeros(1,tN-6) pad*afd(4,N) pbd*bfd(4,N) pau*afu(4,N) pbu*bfu(4,N) -esr(4) -epr(4)];
  As(tN,:)=[zeros(1,tN-6) pad*afd(5,N) pbd*bfd(5,N) pau*afu(5,N) pbu*bfu(5,N) -esr(5) -epr(5)];

  Bs=[zeros(tN-4,1); esi(1); esi(2); esi(4); esi(5);];  % s-polarized incidence 
  rs=linsolve(As,Bs);
  tss=rs(1);  tps=rs(2); rss=rs(tN-1); rps=rs(tN); 
  Bp=[zeros(tN-4,1); epi(1); epi(2); epi(4); epi(5);];  % p-polarized incidence 
  rp=linsolve(As,Bp);
  tsp=rp(1);  tpp=rp(2); rsp=rp(tN-1); rpp=rp(tN); 
